...
function getRandomY() {
    console.log('Getting RandomY...')
    var tempy = Math.floor(Math.random() * window.innerHeight);
...
function insertWordOnPage(word, x, y) {
    ...
    var a = document.createTextNode(word);
    c.appendChild(a);
    c.style.position = 'absolute'
    var containerDiv = document.getElementById('container').appendChild(c);
    c.style.left = x + 'px';
    c.style.top = y + 'px';
...
function findMatchingWord(text_input) {
    ...
    for (i = 0; i < s.length; i++) {
        var element = s[i];
        var word = (s[i]).innerHTML;
        if (word === text_input);
    }
    /*text += randomWordList[r] + "<br>"
  document.getElementById('container').innerHTML = 'text_input';
        s.removeChild(s.childNodes[randomWordList])
        
        
    Part 2 is not working, the console said the error is on line 153. I don't know whats wrong*/
